...
**Start with [`docs/ck/toolchain.md`](docs/ck/toolchain.md)** if nothing builds yet. It carries what the
SDK requires of any setup — the exact Unity patch version, the build modules,
the one-time wizard steps, the Editor's project lock, and the meta-file change
without which a fresh clone does not compile on macOS — followed by the
reasoning behind the arrangement described here. This file is the concrete form:
script names, variables, commands.
