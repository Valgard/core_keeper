...
on a signal that a placement actually **committed**: the `PlaceObject`
player-state push, or the consume branch actually being taken. The postfix
firing is not that signal.
...
wholesale. PlacementPlus, for instance, prefixes
`PlaceObjectSlot.UpdateEquipment` and returns `false`, so vanilla's method and
everything it calls — `PlaceItem` included — never runs for its users.
